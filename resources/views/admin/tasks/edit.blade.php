    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
            <div class="rounded-lg bg-white shadow">
                <form method="POST" action="{{ route('admin.tasks.update', $task) }}" class="space-y-6 px-6 py-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $task->name) }}" placeholder="Enter task name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border px-3 py-2 {{ $errors->has('name') ? 'border-red-500' : '' }}" required>
                        @error('name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4" placeholder="Enter task description" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border px-3 py-2 {{ $errors->has('description') ? 'border-red-500' : '' }}">{{ old('description', $task->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $task->due_date ? $task->due_date->format('Y-m-d') : '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border px-3 py-2 {{ $errors->has('due_date') ? 'border-red-500' : '' }}">
                        @error('due_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="estimated_hours" id="estimated_hours" step="0.5" min="0" value="{{ old('estimated_hours', $task->estimated_hours ?? '') }}" placeholder="0.0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border px-3 py-2 {{ $errors->has('estimated_hours') ? 'border-red-500' : '' }}">
                    ...
                    <div>
                        <label for="actual_hours" class="block text-sm font-medium text-gray-900">Actual Hours</label>
                        <p class="mt-1 flex items-center rounded-md border border-gray-300 bg-gray-50 px-3 py-2 text-sm text-gray-700">
                            {{ $task->actual_hours ?? '0' }} hours
                            <span class="ml-2 text-xs text-gray-500">(Calculated from time entries)</span>
                        </p>
                    </div>
        ...
        <div class="lg:col-span-1 space-y-6">
                            ...
                            <div class="px-6 py-4">
                                <div class="flex items-center justify-between mb-2">
                                    @php
                                        ...
                                        $statusLabel = ucwords(str_replace('-', ' ', $log->new_status));
                                    @endphp
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $statusColors[$log->new_status] ?? 'bg-gray-100 text-gray-800' }}">
                                        {{ $statusLabel }}
                                    </span>
                                </div>
                                @if ($log->user)
            ...
            <!-- Quick Info -->
            <div class="rounded-lg bg-white shadow p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Task Info</h3>
                <div class="space-y-4 text-sm">
                    <div>
                        <p class="text-xs font-medium uppercase tracking-wider text-gray-500">Created</p>
                        <p class="mt-1 text-gray-900">{{ \Carbon\Carbon::parse($task->created_at)->format('M d, Y h:i A') }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-medium uppercase tracking-wider text-gray-500">Last Updated</p>
                        <p class="mt-1 text-gray-900">{{ \Carbon\Carbon::parse($task->updated_at)->format('M d, Y h:i A') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-lg bg-red-50 shadow p-6">
                ...
                <p class="text-sm text-red-800 mb-4">Once you delete this task, there is no going back. Please be certain.</p>
