<x-filament-panels::page>
    ...
    <div class="mb-8 -mt-6 -mx-6 px-6 py-8 bg-gradient-to-br from-amber-50 via-orange-50 to-red-50 dark:from-gray-800 dark:via-gray-900 dark:to-gray-800 border-b border-gray-200 dark:border-gray-700 edison-bg-mesh">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-extrabold bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent tracking-tight">
                    {{ $this->getTitle() }}
                </h1>
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-5 gap-4">
            @php
                $stats = [
                    ...
                    [
                        'label' => 'Blocked',
                        'value' => \App\Models\Task::where('status', 'blocked')->count(),
                        'icon' => 'heroicon-o-x-circle',
                        'color' => 'red',
                    ],
                    [
                        'label' => 'Total Hours',
                        'value' => number_format(\App\Models\Task::sum('estimated_hours'), 1),
                        'icon' => 'heroicon-o-clock',
                        'color' => 'violet',
                    ],
                ];
    ...
    @if($this->getTable()->getRecords()->isEmpty() && !$this->getTable()->getFilters())
        <x-empty-state
            ...
            title="No tasks yet"
            description="Stay organized and on track by creating your first task. Set priorities, track progress, and meet deadlines."
            action-label="Create Task"
            :action-url="route('filament.admin.resources.tasks.create')"
        />
    @else
